...
dat <- arrow::open_dataset(
  sources="/workspace/Projects/ILEC/VBT/Data/ilecdata_20240429"
)
...
dat2[,
     dur_band1:=cut(Duration,
                    breaks=c(0,1,2,3,5,15,25),
                    labels=c("01","02","03","04-05","06-15","16-25"))]

ia_breaks <- c(17,24,34,44,54,64,74,84,99)
...
dat2[,
     Insurance_Plan:=fct_collapse(Insurance_Plan,
                  xL=c("UL","ULSG","VL","VLSG"))]

dat2[,
     Face_Amount_Band:=fct_collapse(Face_Amount_Band,
                                   `01: 0 - 49,999`=c("01: 0 - 9,999",
                                     "02: 10,000 - 24,999",
                                     "03: 25,000 - 49,999"),
                                   `08: 1,000,000+`=c("08: 1,000,000 - 2,499,999",
...
arrow::write_parquet(x=dat3,
     sink="ilec13_17_framework_light.parquet")
